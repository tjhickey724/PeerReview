...
Router.route('/reviewAnswers/:_qid', function () {

	var qid = this.params._qid;
	console.log("Inside reviewAnswers/"+qid);
	var q = Questions.findOne(qid);
	console.log("question:"+JSON.stringify(q));
	var myAns = Answers.findOne({question:qid,createdBy:Meteor.userId()});
	console.log('myAns='+JSON.stringify(myAns));
	// first we look for answers with fewer than 3 reviews
	var toReview = Answers.findOne(
		{question:qid,
		 'myReviewers.20':{$exists:false},
		 createdBy:{$ne:Meteor.userId(),$nin:myAns.myReviews}}
	)
	console.log(JSON.stringify({question:qid,
	 'myReviewers.20':{$exists:false},
	 createdBy:{$ne:Meteor.userId(),$nin:myAns.myReviews}}))
	console.log("toReview="+JSON.stringify(toReview))
	var toReviews;
	if (!toReview) {
    console.log("***** toReviews is empty!");
    ...
    console.log("toReviews = "+toReviews);
		if (toReviews.length==0){
			toReview = undefined;
	...
	console.dir(q);
	console.dir(toReview);
	console.log('Routing to /reviewAnswer2/'+q._id+"/"+toReview._id)
  this.redirect('/reviewAnswer2/'+q._id+"/"+toReview._id);
});



Router.route('/reviewAnswer2/:_qid/:_aid',
 {name:'reviewAnswer',
  data: function(){
		var qid = this.params._qid;
...
Router.route('nothing');

Router.route('/reviewAnswers2/:_qid',
	{name:'nothing2',
		    data: function(){
